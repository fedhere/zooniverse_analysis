...
def get_core_team_labels(zooniverse, multilabel, mulitilabelsubjects,
  ...
  alllabels = ourlabels['countannotation'] > 0 #all with at least one core annotation
  our_rbdf, multilabelsubjects = createRBdf(ourlabels, alllabels, verbose=verbose, readsaved=False, savefile=False)
  ...
  if verbose:
     print("\n\n\n########### the subjects that have >= one core team member labeled")
     display(ourlabels)
...
def create_groundtruth_training(rbdf, date, subjectsfile, verbose=False):
  rbdf_trainingdata = rbdf[rbdf["Real_count"] + rbdf["Bogus_count"] > 20]
  rbdf_trainingdata
...
def getground_truth(filein, date, read_ground_truth=False,
                    verbose=False, plotit=False):
  zooniverse, multilabel, users = readinzooniverse(filein)
  ##### create rb dataframe ######
  rbdf, multilabelsubjects = createRBdf(zooniverse, multilabel,
                                        verbose=verbose, readsaved=True)
  ...
  our_rbdf = get_core_team_labels(zooniverse, multilabel, multilabelsubjects,
                                  coreteamnames,
  ...
  if plotit:
    ...
    return full_ground_truth, [our_rbdf,
                               pd.DataFrame.from_dict(training_ground_truth,
                                                      orient='index',
                                                      columns=['ground_truth']),
                                                      oxfordgt]
