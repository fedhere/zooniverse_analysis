...
def get_core_team_labels(zooniverse, multilabel, mulitilabelsubjects,
  ...
  if verbose:
    ...
    print("\n\n\n########### number of labels by core team member")
    display(ourlabels.groupby("user_name").count())
    print("\n\n\n########### number of objects by number of core team label")
    display(ourlabels.groupby("subjects_id").count())
  ...
  our_rbdf, multilabelsubjects = createRBdf(ourlabels, alllabels, verbose=False, readsaved=False)
  ...
  if verbose:
     print("\n\n\n########### the subjects that more than one core team member labeled")
     display(ourlabels)
...
def user_analysis(filein, date, read_ground_truth=False, verbose=False, plotit=False):
  ...
  if read_ground_truth:
    ...
    ### then core team labels ###
    coreteamnames = ["masao", "bellm", "acero",  "bianco", "brunosanchez"]
    our_rbdf = get_core_team_labels(zooniverse, multilabel, multilabelsubjects, coreteamnames, verbose=False)
    
    ### then oxford labels ### ===> this is cumberson and should be rewritten!
    oxfordgt, _, _ = processclassifications("-atlas-eyeballers-rubin-difference-"
                                          "detectives-classifications.csv", workflow_id=30488, verbose=False)
  
    oxfordgt.replace("Bogus\n","Bogus", inplace=True)
    alllabels = [True]*len(oxfordgt)
    
    oxfordgt, multilabelsubjects = createRBdf(oxfordgt, alllabels, verbose=False, readsaved=False)
    
    oxfordgt["total"] = oxfordgt["Real_count"] + oxfordgt["Bogus_count"]
    oxfordgt = oxfordgt[oxfordgt["total"] > 1]
    oxfordgt["Real_fraction"] = oxfordgt["Real_count"] / (oxfordgt["Real_count"]+oxfordgt["Bogus_count"])
    oxfordgt["Bogus_fraction"] = our_rbdf["Bogus_count"] / (oxfordgt["Real_count"]+oxfordgt["Bogus_count"])
    oxfordgt["agreement"] = oxfordgt[['Real_fraction', 'Bogus_fraction']].max(axis=1)
    oxfordgt = oxfordgt[oxfordgt["agreement"] >= AGREE_THR]
    oxfordgt["ground_truth"] = ["Real" if r>0.5 else "Bogus" for r in oxfordgt["Real_fraction"]]
    
  
    ### merge with also training data ###
